...
- **자르기**: 원본 미리보기 위의 상자를 모바일 사진 자르기처럼 직접 끌어서 범위 지정(모서리=크기, 안쪽=이동), 투명 여백 자르기(+남길 여백)
- **위치 조절**: 고정 캔버스(예: 256×256) 안에 9방향 정렬 + 가로·세로 이동 + 안쪽 여백 + 크기 맞춤
- 여러 장/폴더 일괄 처리, 끌어다 놓기 지원, **저장 이름 직접 지정**(비우면 원본 이름, 여러 장이면 자동 번호)
...
```
CLI 옵션: `-t` 허용 오차, `-f` 경계 부드러움(0=또렷), `-c` 배경색, `--global`, `-r`, `--overwrite`, `-n` 저장 이름(비우면 원본 이름, 여러 장이면 자동 번호)
